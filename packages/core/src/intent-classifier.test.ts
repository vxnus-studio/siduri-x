...
describe('IntentClassifier', () => {
  ...
  describe('Multi-tier Interaction Mode Resolution (Casual, Teach, Hybrid)', () => {
    ...
    test('resolves to teach mode when context.mode is teach', () => {
      const teachContext: RequestContext = { ...dummyContext, mode: 'teach' };
      expect(classifyInputIntent('Remember that my favorite fruit is peach', teachContext).effectiveMode).toBe('teach');
      expect(classifyInputIntent('From now on always be concise', teachContext).effectiveMode).toBe('teach');
      expect(classifyInputIntent('Address me as Operator', teachContext).effectiveMode).toBe('teach');
    });

    test('resolves to casual mode when context.mode is casual', () => {
      const casualContext: RequestContext = { ...dummyContext, mode: 'casual' };
      expect(classifyInputIntent('How are you doing?', casualContext).effectiveMode).toBe('casual');
    ...
    test('explicit context.mode override takes highest precedence', () => {
      ...
      expect(classifyInputIntent('remember my name is Zagin', explicitCasual).effectiveMode).toBe('casual');
