...
describe('Phase 5: Clean-Machine Distribution & E2E Integration Suite', () => {
  ...
  const ALL_CANONICAL_PACKAGES = [
    { filter: '@siduri-x/core', tarName: 'siduri-x-core-1.0.3.tgz', isOrgan: false },
    ...
    { filter: '@siduri-x/voice', tarName: 'siduri-x-voice-1.0.4.tgz', isOrgan: true },
    { filter: '@siduri-x/observation', tarName: 'siduri-x-observation-1.0.2.tgz', isOrgan: true },
    { filter: '@vxnus/siduri', tarName: 'vxnus-siduri-0.1.6.tgz', isOrgan: false },
  ];
  ...
  describe('Phase 5B & 5C & 5G: Clean-Machine Installation & Isolation Acceptance', () => {
    ...
    test('Clean Composition 1: Brain only instance installs, starts, runs doctor and db push', () => {
      ...
      pkgObj.dependencies = {
        '@siduri-x/core': `file:${path.join(tempPackDir, 'siduri-x-core-1.0.3.tgz')}`,
        '@siduri-x/brain': `file:${path.join(tempPackDir, 'siduri-x-brain-1.0.3.tgz')}`,
      };
    ...
    test('Clean Composition 3: Brain + Memory discovers packaged migrations and detects DATABASE_URL', () => {
      ...
      pkgObj.dependencies = {
        '@siduri-x/core': `file:${path.join(tempPackDir, 'siduri-x-core-1.0.3.tgz')}`,
        '@siduri-x/brain': `file:${path.join(tempPackDir, 'siduri-x-brain-1.0.3.tgz')}`,
        '@siduri-x/memory': `file:${path.join(tempPackDir, 'siduri-x-memory-1.0.3.tgz')}`,
      };
