...
export function generateInstanceFiles(options: InstanceGeneratorOptions): GeneratedInstanceFiles {
  ...
  const cliVersion = options.cliVersion || '^1.0.6';
  ...
  const srcIndexJs = [
    ...
    `    res.writeHead(200, { 'Content-Type': 'application/json' });`,
    `    try {`,
    `      const directives = typeof self?.getAllDirectives === 'function' ? await self.getAllDirectives(config.id) : [];`,
    `      const pendingClaims = typeof runtime?.getPendingProposals === 'function' ? await runtime.getPendingProposals(config.id) : [];`,
    `      res.end(JSON.stringify({ claims: pendingClaims, items: [...directives, ...pendingClaims], directives }));`,
    `    } catch (e) {`,
    ...
    `    res.writeHead(200, { 'Content-Type': 'application/json' });`,
    `    try {`,
    `      const proposals = typeof runtime?.getPendingProposals === 'function'`,
    `        ? await runtime.getPendingProposals(config.id)`,
    `        : (typeof self?.getAllDirectives === 'function'`,
    `            ? (await self.getAllDirectives(config.id)).filter((d) => d.status === 'pending')`,
    `            : []);`,
    ...
    `    try {`,
    `      if (typeof self?.resetDirectives === 'function') {`,
    `        await self.resetDirectives(config.id);`,
    `      }`,
    `      if (typeof archive?.reset === 'function') {`,
    `        await archive.reset(config.id);`,
    `      }`,
    `      if (typeof (self as any)?.db?.resetArchive === 'function') {`,
    `        (self as any).db.resetArchive(config.id);`,
    `      }`,
