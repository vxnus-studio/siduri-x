...
export function validateOrganManifest(manifest: unknown, sourcePath?: string): OrganManifest {
  ...
  if (!m.name || typeof m.name !== 'string' || !m.name.startsWith('@siduri-y/')) {
    throw new Error(`Invalid manifest name in ${sourcePath || 'unknown'}: expected package name starting with @siduri-y/`);
  }
