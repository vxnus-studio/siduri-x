...
11. **Clean Distribution Packaging & Smoke Test**:
    `npm run release:check` passes on all 14 packages (13 canonical `@sidurijs/*` domain and organ packages + `@vxnus/siduri` CLI; zero leaking `workspace:` references). `clean-machine-smoke.test.ts` validates that tarballs unpack and scaffold an empty project in an isolated temp folder (packaging smoke test).
