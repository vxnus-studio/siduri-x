...
15. **Companion Server Template Request Body Parsing (P0)**:
    Standalone companion instances generated by `@vxnus/siduri` parse POST body JSON payloads before accessing `payload.context` or dispatching `/chat` and `/chat/stream` requests, preventing `ReferenceError: payload is not defined` runtime failures.
