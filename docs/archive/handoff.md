...
1. **Phase 3A: API Contract Parity (COMPLETED)**
   ...
   - Updated `MemoryOrgan` interface in `@siduri-y/core` to expose these methods.
...
2. **Phase 3B: Web UI implementation (COMPLETED)**
   ...
   - Adapted the Overlay client to correctly parse the newer `@siduri-y/body` WebSocket event schema (`type: "speech" | "state_transition" | "lifecycle"`) instead of relying on the legacy `response_plan` event, properly simulating amplitude where no longer explicitly provided.
