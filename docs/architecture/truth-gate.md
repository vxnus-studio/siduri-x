...
- **Operator Channel & Untrusted Evidence Protection**:
  - Responses generated in sensitive channels (`operator`) or incorporating untrusted evidence (e.g., OCR, unverified sensory readings) automatically require operator approval (`requiresApproval: true`).
- **Gate Evaluation (`evaluateGate`)**:
  - If approval is pending or evidence sensitivity violates disclosure rules, the response is held (`status: 'STAGED'`, `reasonCode: 'APPROVAL_REQUIRED'`). Voice synthesis and body execution are inhibited until an operator submits `/dev/approve-response`.
- **Evidence Disclosure**: Private or restricted evidence attached to candidate speech is filtered before external emission, ensuring that internal secrets never leak across audience boundaries.
