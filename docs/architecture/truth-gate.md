...
1. **Self**: Identity, archetype, core directives, and qualitative relational stances (`@sidurijs/self`).
2. **Knowledge (The Life Database)**: The user's sovereign objective reality — entities, events, tasks, schedule, and grounded preferences (`@sidurijs/knowledge`).
3. **Memory**: Long-term semantic claims and episodic interaction traces (`@sidurijs/memory`).
4. **Action & Expression**: Speech disclosure, physical actuation, and tool execution (`@sidurijs/brain`, `@sidurijs/hands`).
...
The Truth Gate is implemented across `@sidurijs/core`, `@sidurijs/knowledge`, `@sidurijs/self`, and `@sidurijs/memory`.
...
- **Active Self Isolation**: In the `@sidurijs/self` compiler (`ActiveSelfCompiler`), pending directives are excluded under the `pending_not_active` invariant diagnostic.
...
In addition to state persistence, the Truth Gate enforces runtime admissibility before Siduri emits speech or executes actions (see `@sidurijs/core`):
...
- **Staged Response Plans**: Candidate outputs generated by `@sidurijs/brain` pass through `ResponseGatingEngine.stageResponse()`.
